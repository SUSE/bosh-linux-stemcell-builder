...
elif [ "${os_type}" == "sles" -o "${os_type}" == "opensuse" ]
then
  # Copy google daemon packages into chroot
  cp -R $assets_dir/google-centos/*.rpm $chroot/tmp/google/

  run_in_chroot $chroot "zypper install -y python-setuptools python-boto"

  run_in_chroot $chroot "zypper --no-gpg-checks install -y /tmp/google/*.rpm"
